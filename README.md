...
4. Опоздания шлются в единый чат: `DEFAULT_TELEGRAM_CHAT_ID`. Главный администратор (чей ID указан в `DEFAULT_TELEGRAM_CHAT_ID`) может динамически добавлять и удалять другие чаты для рассылки с помощью команд `/addchat`, `/delchat` и `/chats`. (Чаты сохраняются в `chats.json`).
